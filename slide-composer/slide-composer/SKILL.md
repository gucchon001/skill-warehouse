...
name: slide-composer
description: |
  マークダウンからスライドを生成・変換する。Marp を主軸に Markdown → PDF / HTML / PPTX へ変換する手順、スライド構成のベストプラクティス、テーマ設定を扱う。
  Triggers: 「マークダウンをスライドに」「MDをPPTXに変換」「Marpでスライド作成」「スライドを作って」「プレゼン資料をMarkdownから生成」「slide from markdown」「presentation from md」「marp」「convert md to pptx」「marp cli」
metadata:
