...
description: "BigQuery MCP ツール選択・呼び出し原則（execute_sql, list_*, ask_data_insights 等）。設定・接続は bigquery-mcp。Triggers: BigQuery でクエリ, テーブル一覧, データ分析, 予測, BQ MCP ツール."
metadata:
...
**親スキル**: **bigquery-mcp** で入口を確認（本スキルは **ツールの選び方・パラメータ**のみ）。
...
| スキル | 役割 |
|--------|------|
| **bigquery-mcp** | **入口・分岐**・接続トラブル（`references/setup-and-troubleshooting.md`） |
| **bigquery-mcp-install** | 初回セットアップ（バイナリ・`mcp.json`） |
