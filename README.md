...
5. [品質・デバッグ・UI / UX](#5-品質デバッグui--ux) … [UI / UX 定義のみ](#ui-ux-definition)
...
### UI / UX の定義（本ライブラリ内の共通言語） {#ui-ux-definition}

スキルを選ぶ・レビューするときの **UI** と **UX** の切り分け。プロジェクト固有ドキュメントではなく、**グローバルスキル**の責務整理としてここに固定する。

**UI（User Interface）**  
画面上の**見えるもの・触れる部品**の一貫性：レイアウト、タイポグラフィ、色・余白、コンポーネントの形状、インタラクション状態（hover / focus / disabled / エラー）、レスポンシブ、ローディングの見た目。単一画面・部品レベルの視覚・設計が中心。

**UX（User Experience）**  
**タスク達成までの流れ**と**情報の優先順位**：導線、画面遷移、一覧→詳細の順序、フォームの段階、エラー時に「次に何をすればよいか」、入口で何を最初に見せるか（ダッシュボードのブロック順など）。複数画面・前後の文脈が中心。

**重なり**  
アクセシビリティ（コントラスト、フォーカス、ラベル）は UI の見た目と UX の利用しやすさの両方に関わる。

**対応スキル（読み分け）**

| 主に扱う | スキル | 使うとき |
|----------|--------|----------|
| **UI** | [**ui-frontend-patterns**](./ui-frontend-patterns/) | コンポーネントを揃える・階層・CTA・コントラスト・モバイル・チェックリスト |
| **UX**（入口・並び） | [**dashboard-first-view-ux**](./dashboard-first-view-ux/) | トップ・ダッシュボードのファーストビュー・「自分宛て」優先・説明の二次化 |

プロジェクト側に `design-ui` 等のハブスキルがある場合は、そこから上記へ辿ってもよい。
