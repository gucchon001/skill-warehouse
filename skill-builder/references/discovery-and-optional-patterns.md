...
### プロジェクトメモリ・プロジェクト指示ファイル

プロジェクトに **キーワード → スキル名** の対応表を書く運用は、**チーム内の一貫性**には有効。ファイル名・仕様は **利用製品の Memory / プロジェクト指示のドキュメント**に従う（例として `CLAUDE.md` という名が使われることがあるが、**全製品共通の必須要件ではない**）。

### ルールファイル・ユーザープロンプト

ルール（例: `.mdc`）やプロンプトで「〇〇のときはスキル X を読め」と書くのも**運用**。スキル仕様の**代替にはならない**（`description` の整備が先）。
